...
class AuthenticationHelper {
     ...
     * @throws Exception 
     */
    JSONObject PerformSRPAuthentication(String username, String password) throws Exception {
        ...
        try {
            ...
            if (ChallengeNameType.PASSWORD_VERIFIER.toString().equals(initiateAuthResult.getChallengeName())) {
             ...
             // authresult = result.getAuthenticationResult().getAccessToken();
                ...
                System.out.println("\nID TOKEN\t"+result.getAuthenticationResult().getIdToken());
                authresult.put("id_token",result.getAuthenticationResult().getIdToken());
                authresult.put("access_token",result.getAuthenticationResult().getAccessToken());
                authresult.put("refresh_token",result.getAuthenticationResult().getRefreshToken());
            }
        } catch (final Exception ex) {
            System.out.println("\nException \t" + ex);
			authresult.put("Exception",ex.toString());
        }
        return authresult;
    }
