	...
	public JSONObject handleRequest(JSONObject input, Context context) {
		...
		try {
			...
			if (select_resultSet.next()) {
				 ...
				 * 
				 * str_msg =
				 * "Forgot password functionality cannot be executed as user is not yet confirmed"
				 * ; jo_change_psw.put("status", "0"); jo_change_psw.put("message", str_msg);
				 * jo_change_psw.put("id", select_resultSet.getInt("id")); return jo_change_psw;
				 * } else {
				 */
				// --RESET REQUESTED----//
